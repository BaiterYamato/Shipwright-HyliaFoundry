...
typedef int32_t (*FoundryHostGetAssetPathFn)(void* userData, const char* assetId, char* outPath, uint32_t outPathSize);
typedef int32_t (*FoundryHostGetAssetInfoFn)(void* userData, const char* assetId, char* outKind, uint32_t outKindSize,
                                             char* outFormat, uint32_t outFormatSize, uint64_t* outSizeBytes);
typedef int32_t (*FoundryHostQueryPublicJsonFn)(void* userData, const char* requestJson, char* outJson,
                                                uint32_t outJsonSize);
typedef int32_t (*FoundryHostCallServiceFn)(void* userData, const char* targetModId, const char* serviceId,
                                            const char* requestJson, char* outJson, uint32_t outJsonSize);
typedef int32_t (*FoundryHostInvokeActionJsonFn)(void* userData, const char* actionJson, char* outJson,
                                                 uint32_t outJsonSize);
...
typedef struct FoundryHostApiV1 {
    ...
    FoundryHostGetAssetPathFn getAssetPath;
    FoundryHostGetAssetInfoFn getAssetInfo;
    FoundryHostQueryPublicJsonFn queryPublicJson;
    FoundryHostCallServiceFn callService;
    FoundryHostInvokeActionJsonFn invokeActionJson;
} FoundryHostApiV1;
