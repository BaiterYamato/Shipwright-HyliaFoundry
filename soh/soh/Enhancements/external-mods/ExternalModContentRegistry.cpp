...
namespace {
template <typename T> const T* FindById(const std::vector<T>& entries, const std::string& id) {
    const auto it = std::find_if(entries.begin(), entries.end(), [&id](const T& entry) { return entry.id == id; });
    if (it == entries.end()) {
        return nullptr;
    }
    return &(*it);
...
const ExternalModStatusDefinition*
ExternalModContentRegistry::FindStatusDefinitionById(const ExternalModRuntime& runtime, const std::string& statusId) {
...
const ExternalModTargetingProfile*
ExternalModContentRegistry::FindTargetingProfileById(const ExternalModRuntime& runtime, const std::string& profileId) {
...
const ExternalModProjectileProfile*
ExternalModContentRegistry::FindProjectileProfileById(const ExternalModRuntime& runtime, const std::string& profileId) {
...
ExternalModContentRegistry::FindAimCameraProfileById(const ExternalModRuntime& runtime, const std::string& profileId) {
...
void ExternalModContentRegistry::SortPackagesByPriority(std::vector<ExternalModPackage*>& packages) {
    std::sort(packages.begin(), packages.end(), [](const ExternalModPackage* lhs, const ExternalModPackage* rhs) {
